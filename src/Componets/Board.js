      ...
      <>
      {
        board.map((row, indexY) => {
         return (<Row>
          ...
          row.map((col, indexX) => {
            //{console.log("BOARD RERENDER!!")}
            return (<Square selected = {col.selected} bgc={col.player === 1 ? "#339933" : col.player === -1 ? "#222222" : "#993333"} onSelectPiece={onSelectPiece} position = {[col.x,col.y]} graphic={`${col.graphic}${col.x},${col.y}`}></Square>)
          })
            }
          </Row>)
          })
